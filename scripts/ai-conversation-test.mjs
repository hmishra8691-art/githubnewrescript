...
console.log("\nSTUDIO — Branding → AI Conversational Survey: simple controls, presets, locale, lint, advanced tree");
{
  const r = await fetch(`${process.env.STUDIO_URL ?? "http://localhost:3000"}/api/ai/rephrase`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text: "x" }) });
  assert.equal(r.status, 200, `Studio must be started with AI_API_URL=fake: — got ${r.status}`);
}
await h.loadDef(survey({ enabled: false }));
await h.goTab("Branding");
await h.page.waitForSelector('[data-testid="ai-enabled"]');
assert.ok(!(await h.page.$('[data-testid="ai-voice-simple"]')), "voice controls appear only when voice is on");
await h.page.click('[data-testid="ai-enabled"]');
await h.page.selectOption('[data-testid="ai-interaction"]', "text_voice");
await h.page.selectOption('[data-testid="presentation-mode"]', "adaptive");
await h.page.waitForSelector('[data-testid="ai-voice-simple"]');
await h.page.selectOption('[data-testid="ai-voice-preset"]', "friendly_consumer");
await h.page.selectOption('[data-testid="ai-country"]', "IN");
await h.page.selectOption('[data-testid="ai-language"]', "en");
assert.equal(await h.page.inputValue('[data-testid="ai-dialect"]'), "en-IN", "country + language → the country's dialect for that language");
let lintText = await h.page.$$eval('[data-testid="ai-lint"]', (els) => els.map((e) => e.textContent).join("\n"));
assert.match(lintText, /no objective is written/, "adaptive + stay-within-objective without an objective is flagged, in words");
await h.page.click('[data-testid="ai-advanced"]');
await h.page.waitForSelector('[data-testid="ai-advanced-panel"]');
await h.page.fill('[data-testid="ai-objective"]', "the store experience");
await h.page.fill('[data-testid="ai-restricted"]', "health, income");
await h.page.fill('[data-testid="ai-max-followups"]', "1");
await h.page.fill('[data-testid="ai-pronunciations-term"]', "Miures");
await h.page.fill('[data-testid="ai-pronunciations-said"]', "Mee-yoo-res");
await h.page.click('[data-testid="ai-pronunciations-add"]');
await h.page.click('[data-testid="ai-rule-add"]');
await h.page.waitForSelector('[data-testid="ai-rule"]');
await h.page.fill('[data-testid="ai-rule-max"]', "2");
await h.page.waitForTimeout(300);
lintText = await h.page.$$eval('[data-testid="ai-lint"]', (els) => els.map((e) => e.textContent).join("\n"));
assert.ok(!/no objective/.test(lintText), "the lint cleared once the objective was written");
assert.match(await h.page.textContent('[data-testid="ai-voice-test-voice"]'), /en-IN/, "the voice test panel names the locale it would speak");
let def = await h.readDef();
const ac = def.branding.aiConversation;
assert.equal(ac.enabled, true);
assert.equal(ac.interaction, "text_voice");
assert.equal(ac.conversation, "adaptive");
assert.equal(ac.adaptive.enabled, true, "choosing Adaptive switched adaptive follow-ups on");
assert.equal(ac.adaptive.maxFollowUps, 1);
assert.equal(ac.adaptive.researchObjective, "the store experience");
assert.deepEqual(ac.adaptive.restrictedTopics, ["health", "income"]);
assert.equal(ac.adaptive.rules.length, 1);
assert.equal(ac.adaptive.rules[0].maxFollowUps, 2);
assert.equal(ac.voice.preset, "friendly_consumer");
assert.equal(ac.voice.profile.personality, "warm", "the preset set the personality…");
assert.equal(ac.voice.audio.rate, 0.95, "…and the pace");
assert.deepEqual({ c: ac.voice.locale.country, l: ac.voice.locale.language, d: ac.voice.locale.dialect }, { c: "IN", l: "en", d: "en-IN" });
assert.deepEqual(ac.voice.pronunciations, { Miures: "Mee-yoo-res" });
assert.equal(def.branding.layout.presentation, "conversational", "older field mirrored");
assert.deepEqual(def.branding.layout.voice, { readAloud: true, dictation: true, lang: "en-IN" }, "older voice fields mirrored");
assert.equal(def.questions.length, 5, "no question changed");
console.log("  ok   one object, branding.aiConversation, from simple + advanced controls; preset applied; lint reacts; legacy fields mirrored");

console.log("\nSTUDIO — Properties → AI conversation: overrides, spoken script with an AI version to approve, option spoken labels, speech preview, simulator");
await h.goTab("Questions");
await h.page.click('[data-qid="q1"]');
await h.page.waitForSelector('[data-testid="psec-head-ai-conversation"]');
if (!(await h.page.$('[data-testid="ai-spoken-mode"]'))) await h.page.click('[data-testid="psec-head-ai-conversation"]');
await h.page.waitForSelector('[data-testid="ai-spoken-mode"]');
let scriptItems = await h.page.$$eval('[data-testid="ai-speech-script"] li', (els) => els.map((e) => e.textContent));
assert.match(scriptItems[0], /How was your visit to Mee-yoo-res\?/, "the speech preview applies the survey's pronunciations");
assert.ok(scriptItems.some((t) => /option 3:.*Not good/.test(t)), "…and the option's spoken label");
await h.page.selectOption('[data-testid="ai-spoken-mode"]', "ai");
await h.page.waitForSelector('[data-testid="ai-spoken-generate"]');
assert.ok(await h.page.isDisabled('[data-testid="ai-spoken-generate"]'), "generation is off until the survey allows AI rephrasing");
await h.goTab("Branding");
await h.page.waitForSelector('[data-testid="ai-advanced"]');
if (!(await h.page.$('[data-testid="ai-rephrase"]'))) await h.page.click('[data-testid="ai-advanced"]');
await h.page.click('[data-testid="ai-rephrase"]');
await h.goTab("Questions");
await h.page.click('[data-qid="q1"]');
await h.page.waitForSelector('[data-testid="ai-spoken-generate"]');
await h.page.click('[data-testid="ai-spoken-generate"]');
await h.page.waitForFunction(() => (document.querySelector('[data-testid="ai-spoken-ai-version"]')?.value ?? "").length > 0, null, { timeout: 10000 });
const aiVersion = await h.page.inputValue('[data-testid="ai-spoken-ai-version"]');
assert.match(aiVersion, /How was your visit to Miures\?/, "the (fake) provider proposed a spoken version");
assert.ok(!(await h.page.isChecked('[data-testid="ai-spoken-approve"]')), "…not approved yet: the displayed text is what is spoken until then");
scriptItems = await h.page.$$eval('[data-testid="ai-speech-script"] li', (els) => els.map((e) => e.textContent));
assert.match(scriptItems[0], /How was your visit to Mee-yoo-res\?/, "unapproved → the displayed text is still what the preview reads");
await h.page.fill('[data-testid="ai-spoken-ai-version"]', "Tell me, how was your visit to Miures?");
await h.page.click('[data-testid="ai-spoken-approve"]');
await h.page.waitForTimeout(200);
scriptItems = await h.page.$$eval('[data-testid="ai-speech-script"] li', (els) => els.map((e) => e.textContent));
assert.match(scriptItems[0], /Tell me, how was your visit to Mee-yoo-res\?/, "approved → the AI version is spoken, pronunciations still applied");
await h.page.click('[data-testid="ai-spoken-lock"]');
await h.page.waitForTimeout(200);
scriptItems = await h.page.$$eval('[data-testid="ai-speech-script"] li', (els) => els.map((e) => e.textContent));
assert.match(scriptItems[0], /^question: How was your visit to Mee-yoo-res\?/, "locked → exactly the displayed wording, whatever was generated");
assert.ok(await h.page.isDisabled('[data-testid="ai-spoken-generate"]'), "…and no more generation");
await h.page.click('[data-testid="ai-spoken-lock"]');
await h.page.click('[data-testid="ai-spoken-options"] summary');
await h.page.fill('[data-testid="ai-option-spoken-1"]', "Really great");
await h.page.selectOption('[data-testid="ai-q-interaction"]', "text");
await h.page.fill('[data-testid="ai-q-max-followups"]', "0");
await h.page.waitForTimeout(300);
scriptItems = await h.page.$$eval('[data-testid="ai-speech-script"] li', (els) => els.map((e) => e.textContent));
assert.match(scriptItems[0], /Tell me, how was your visit to Mee-yoo-res\?/, "unlocked again → the approved AI version");
assert.ok(scriptItems.some((t) => /Really great/.test(t)), "the option's spoken label is read");
def = await h.readDef();
const q1 = def.questions.find((q) => q.id === "q1");
assert.equal(q1.text, "How was your visit to Miures?", "the DISPLAYED text never changed");
assert.deepEqual({ mode: q1.spoken.mode, ai: q1.spoken.aiVersion, ok: q1.spoken.aiApproved, locked: q1.spoken.locked }, { mode: "ai", ai: "Tell me, how was your visit to Miures?", ok: true, locked: false });
assert.equal(q1.options[0].spoken, "Really great");
assert.equal(q1.options[0].label, "Great", "the stored label is untouched");
assert.deepEqual(q1.ai, { interaction: "text", adaptive: { maxFollowUps: 0 } }, "per-question overrides stored as overrides only");
await h.goTab("Questions");
await h.page.click('[data-qid="q2"]');
await h.page.waitForSelector('[data-testid="ai-simulator"]');
assert.match(await h.page.textContent('[data-testid="ai-simulator"]'), /Research objective: the store experience/, "the simulator shows the instruction the follow-up writer receives");
await h.page.fill('[data-testid="ai-sim-answer"]', "The queue was very long");
await h.page.click('[data-testid="ai-sim-run"]');
await h.page.waitForSelector('[data-testid="ai-sim-result"]');
assert.match(await h.page.textContent('[data-testid="ai-sim-result"]'), /You mentioned “The queue was very long”/, "the follow-up the interviewer would ask, from the runtime's probe writer");
console.log("  ok   AI version proposed → approved; lock = displayed wording; option spoken label; overrides; displayed text and labels untouched; simulator");
