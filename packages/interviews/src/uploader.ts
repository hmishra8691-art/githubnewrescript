...
export class RecordingUploader {
  ...
  constructor(opts: UploaderOptions) {
    ...
    this.clientToken = `${opts.responseId}:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 8)}`;
    this.state = {
   ...
   */
  async finish(durationSeconds: number): Promise<{ ok: true; mediaId: string } | { ok: false; error: string }> {
    ...
    return { ok: true, mediaId: reply.mediaId };
  }
