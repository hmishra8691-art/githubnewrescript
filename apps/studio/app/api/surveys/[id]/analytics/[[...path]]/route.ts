...
export async function POST(req: NextRequest, { params }: { params: { id: string; path?: string[] } }) {
  ...
  if (head === "export") {
    ...
    /*
     * §43 — pictures the client rendered for blocks PowerPoint cannot draw
     * (maps). Every one is checked before it goes near the deck: a PNG or
     * JPEG data URI, within a size cap, and no more than a sensible number of
     * them. An export is a document the customer forwards, not a channel for
     * arbitrary bytes, so anything failing the check is dropped and the slide
     * falls back to the chart it would have drawn anyway.
     */
    const images: Record<string, string> = {};
    const sent = (body.images ?? {}) as Record<string, unknown>;
    for (const [blockId, uri] of Object.entries(sent).slice(0, 40)) {
      if (typeof blockId === "string" && blockId.length <= 64 && isEmbeddableImage(uri)) images[blockId] = uri;
    }
    const buf = format === "pptx" ? await buildPptx({ report: { ...report, author: report.author ?? ctx.user.fullName }, results, theme, settings: settings as never, meta, images }) : await buildXlsx({ report, results, theme, settings: settings as never, meta: { Survey: ctx.survey.title, "Generated by": ctx.user.fullName || ctx.user.email } });
