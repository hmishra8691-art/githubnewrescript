...
A transfer is not an administrative act. The person whose credits they are moves them too, through the same SQL function and the same two ledger lines under one transfer id — what was missing was a door, not an engine.

**Whose credits may move**: a person's own wallet needs no capability, because it is theirs. A project's wallet needs **`billing.transfer`** on that project — a new capability granted to the **owner** alone, so an editor who programs the survey cannot move the budget. `POST /api/billing/transfer` resolves the source through `requireProjectFor(user, id, "billing.transfer")`; a project someone merely collaborates on is refused with a sentence saying so.

**What may move**: `balance − reserved`, enforced by `Meter.transfer` and re-implemented nowhere — money an operation in flight is holding is not theirs to give away, and neither is overdraft room. The screen shows the same number the server checks, so a refusal is never a surprise.

**Who receives**: another person **by User ID** (`USR-10482`, or their email), resolved within the sender's own workspace — the reach project sharing already has, not a lookup oracle for the installation — or one of the sender's own projects. A transfer to oneself is refused (`self_transfer`), as is one whose source and destination are the same wallet.

`POST { action: "resolve", userCode }` names the recipient before anything moves, so a mistyped ID is caught in the confirmation rather than discovered afterwards: *"You are about to transfer $25.00 to User USR-10482 (Ana Costa). This amount will be deducted from your available credits and added to the recipient's credits."*

**UI** `/billing` (My usage) → **Transfer credits**. **History** is per person and from their point of view — `GET /api/billing/transfer` → Date · Transfer ID · Type (Sent / Received) · Recipient or sender · Amount · Status, covering their own wallet and the projects they own. Administrators keep the system-wide view above. Tests: cases A–D in `packages/billing/src/billing.test.ts`; `scripts/billing-test.mjs` asserts the route is session-guarded.

### On the projects dashboard

Every project card carries its own meter — Wallet · Used · Remaining · Meter used, a bar, and the level word — so a researcher sees which project is about to stop without opening any of them. `GET /api/billing/projects` answers the whole page in **one request and two store reads** (`apps/studio/lib/projectMeters.ts`), and the arithmetic is `projectMeter()` in `packages/billing/src/wallet.ts`, beside `summarizeWallet`, so the card and the project's own Usage tab cannot disagree. Customer charges only: `ProjectMeter` has no cost field. **Refill wallet** on the card transfers from the person's own credits (or, for a platform administrator, assigns new ones) through the existing atomic paths — it never writes a balance. Full account: `claude/projects-dashboard-wallets-and-cloning.md`.
